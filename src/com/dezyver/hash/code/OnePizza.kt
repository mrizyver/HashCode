...
fun selection(solutions: List<EstimatedSolution>): List<EstimatedSolution> {
    ...
    return solutions.filter {
        val probability =
            (it.score.toFloat() - worstScore) / (bestScore.toFloat() - worstScore) + 0.1f
...
fun groupWishes(wishes: List<Wish>): List<Wish> {
    val sortedWishes =
        wishes.sortedWith { o1, o2 -> (o1.dislikes.size - o1.likes.size) - (o2.dislikes.size - o2.likes.size) }
    val groupedWishes = ArrayList<Wish>()
    client@ for (wish in sortedWishes) {
        group@ for ((index, group) in groupedWishes.withIndex()) {
            val likesConflict: Boolean =
                wish.likes.fold(false) { acc, str -> group.dislikes.contains(str) || acc }
            val dislikesConflict: Boolean =
                wish.dislikes.fold(false) { acc, str -> group.likes.contains(str) || acc }
            if (!likesConflict && !dislikesConflict) {
                groupedWishes[index] =
                    Wish(group.likes + wish.likes, group.dislikes + wish.dislikes)
                continue@client
            }
        }
...
fun parseInput(file: File): List<Wish> {
    ...
    while (num * 2 > index) {
        ...
        output.add(
            Wish(
                liked.subList(1, liked.size).toSet(),
                disliked.subList(1, disliked.size).toSet()
            )
        )
    }
    return output
...
fun calculateScore(wishes: Collection<Wish>, recipe: Set<String>): Int {
    return wishes.fold(0) { score, wish ->
        val wishFits = recipe.containsAll(wish.likes) && !wish.dislikes.fold(false) { b, dis ->
            recipe.contains(dis) || b
        }
